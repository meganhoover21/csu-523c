...
```{r}
#| eval: false
attributes<- download.file('https://zenodo.org/records/15529996/files/camels_attributes_v2.0.pdf?download=1',
...
`zero_q_freq` represents frequency of days with Q = 0 mm/day
...
# Generate the split
camels_split <- initial_split(camels, prop = 0.8) #80% for training
...
#Same coefficients, Same R² (0.7697),Same residuals

#what it means?
#interaction (+0.10)= rainfall’s effect slightly depends on aridity
#p_mean (+1.48) → more rainfall → higher flow
#aridity (-0.88) → higher aridity → lower flow
```
...
```{r}
#| error: true
nrow(camels_test) #135
nrow(camels_train) #536
...
autoplot(wf) #rand_forest has slightly higher rsq and lower rmse
...
Although performance differences are relatively small across models, the neural network consistently achieved the highest R² and lowest RMSE, suggesting slightly better generalization. This improvement is likely due to its flexibility in capturing nonlinear relationships between predictors and streamflow. Therefore, it is the most appropriate model to carry forward.
...
xgb_model <- boost_tree() %>%
  set_engine("xgboost") %>%
  set_mode("regression") #bc predicting streamflow
```
...
added to previous workflow, refit against samples, and comparing all four models
```{r}
wf <- workflow_set(
  list(rec),
  list(lm = lm_model, rf = rf_model, xgb = xgb_model, nn = nn_model)) %>%
  workflow_map('fit_resamples', resamples = camels_cv)
...
vis_dat shows that in the camels dataset there are many na values in the geol_2nd_class, some within the q_mean, a few in the geol_porosity and root depths, and one across several other end columns. Skimr shows that many variables are skewed right or left, and only some look equally distributed. Numeric and character variables are what make up the dataframe. These patterns suggest the need for transformations (e.g., log scaling) to reduce skewness, as well as careful handling of missing values and potential encoding of categorical predictors in the modeling workflow.
 
visualize skewed data: These patterns indicate non-normality in key predictors and the response variable, supporting the use of log transformations to reduce skewness and improve model performance.
```{r}
#skewed right should use step_log()
ggplot(camels, aes(x = q_mean)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribution of Mean Streamflow (q_mean)")

# Looks bimodal but mosly skewed right
ggplot(camels, aes(x = p_mean)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribution of Precipitation (p_mean)")
...
#skewed right
ggplot(camels, aes(x = sand_frac)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribution of sand")

#skewed right
ggplot(camels, aes(x = slope_mean)) +
  geom_histogram(bins = 30) +
  labs(title = "Distribution of sand")

```

Initial split and extraction:
```{r}
set.seed(123)
...
Stratification was not used because logQmean is continuous and does not have clear class groupings. While stratification could be approximated by binning variables such as aridity into quartiles, this introduces arbitrary thresholds.
...
## Recipe (30) — *Week 5-1*
 ...
 Define formula for predicting `logQmean`
```{r}
logQmean ~ aridity + p_mean + elev_mean + slope_mean + sand_frac
```
 
**Justify choice:**
The selected predictors represent key physical controls on streamflow. Precipitation (p_mean) provides water input, while aridity reflects the balance between water supply and evapotranspiration, making them main drivers of runoff generation. Elevation (elev_mean) influences temperature and precipitation patterns, while slope (slope_mean) affects runoff velocity and drainage. Sand fraction (sand_frac) was included because it controls soil permeability and infiltration, influencing how water is stored or runoff.

Exploratory data analysis showed that q_mean, p_mean, sand_frac, slope_mean, and aridity are strongly right-skewed, supporting the use of log transformations to improve linearity and reduce the influence of extreme values. Missing values were observed across several predictors, justifying the removal of incomplete cases to ensure consistent model fitting. Because predictors are measured on different scales (e.g., precipitation vs. slope), normalization is applied to ensure fair contribution across models. Finally, an interaction between aridity and precipitation is included to capture their coupled influence on streamflow, as suggested by the nonlinear patterns observed in the EDA.
...
rec <- recipe(
  logQmean ~ aridity + p_mean + elev_mean + slope_mean + sand_frac,
  data = camels_train) %>%
  # Address right-skewed predictors observed in EDA
  step_log(aridity, p_mean, slope_mean, sand_frac, offset = 1) %>%
  # Capture interaction between climate variables
  step_interact(terms = ~ aridity:p_mean) %>%
  # Remove rows with missing values (identified in vis_dat)
  step_naomit(all_predictors(), all_outcomes()) %>%
  # Standardize predictors (different units/scales)
...
Random forests are appropriate for this problem because they can capture nonlinear relationships and interactions between watershed characteristics and streamflow without requiring strict assumptions about variable distributions.

A radial basis function support vector machine is appropriate because it can model smooth nonlinear relationships between environmental predictors and streamflow while remaining robust to complex feature interactions.

Neural networks are useful for this regression problem because they can model highly complex and nonlinear relationships between climate, topography, soils, and streamflow response. We also saw how well they did in above model comparisons.
...
```{r}
#random cv estimates model performance when observations are randomly mixed across folds.
cv_random <- vfold_cv(camels_train, v = 10)

#training data to sf.converts the basin coordinates into a spatial object so folds can be separated geographically rather than randomly.
camels_train_sf <- st_as_sf(
  camels_train,
  coords = c("gauge_lon", "gauge_lat"),
  crs = 4326)

#spatial block cv reduces spatial leakage by ensuring geographically nearby basins are grouped together within folds.
cv_spatial <- spatial_block_cv(
  camels_train_sf,
  v = 10)
...
#fit over random cv. code based off lecture
wf_random <- wf %>%
  workflow_map(
    "fit_resamples",
    resamples = cv_random,
...
Random CV:
```{r}
#bag_mlp had highest rsq (avg= 92.75),rand_forest lowest at avg= 91.5.
#bag_mlp had lowest rmse ~ avg 0.340 and svm_rbf highest at ~ avg 0.359
autoplot(wf_random)

#neural network did best, rf did worst
rank_results(
  wf_random,
  rank_metric = "rsq",
  select_best = TRUE)
...
Spatial CV:
```{r}
#bag_mlp does the best, rand_forest is 2nd, and svm_rbf is worst
autoplot(wf_spatial)

#neural netwprk is best, rf is 2nd and, svm is worst
rank_results(
  wf_spatial,
  rank_metric = "rsq",
  select_best = TRUE)
...
The neural network (bag_mlp) performed best under both random and spatial cross-validation, with the highest average R² and lowest RMSE values across both resampling schemes. Under random CV, the neural network achieved an R² of about 0.927 and an RMSE of 0.340, while under spatial CV the R² decreased to about 0.883 and RMSE increased to 0.375. This performance drop was expected because spatial cross-validation reduces spatial leakage by separating geographically nearby basins into different folds, creating a more realistic test of model transferability. The random forest and SVM models showed similar declines in performance, though both remained reasonably strong predictors overall. The spatial CV estimates are more trustworthy as measures of true generalization because they better represent prediction in unseen geographic regions rather than nearby basins with similar environmental characteristics. The relatively moderate drop in R² (~0.04) suggests that the predictors contain meaningful transferable hydrologic information rather than relying primarily on geographic proximity. The spread in per-fold metrics, particularly the larger standard errors under spatial CV, indicates that model performance varies across regions and that some watersheds are more difficult to predict due to differing climate, topography, or hydrologic characteristics not fully captured by the training data.
...
A regular grid was used to evaluate combinations of hidden layer size and regularization strength across a broad but interpretable parameter space.
...
show_best(nn_tuned, metric = "rsq")

select_best(nn_tuned, metric = "rsq")
```
 
 - [x ] In 2–3 sentences, describe which hyperparameters mattered most and what you learned about the model's sensitivity
 
The tuning results showed that both the number of hidden units and the penalty parameter influenced model performance, though the model was somewhat more sensitive to hidden layer size. Networks with 5–7 hidden units consistently produced the highest R² values, suggesting that moderate model complexity captured the nonlinear hydrologic relationships best. Smaller penalty values also performed better, indicating that only light regularization was needed and that overly strong penalties reduced the model’s ability to learn important predictor relationships.
...
```{r}
# permutation importance object
#nn_vi <- vip::vi_permute(
#  object = extract_fit_parsnip(final_nn_fit)$fit,
 ...
 # feature_names = c(
  ...
  #train = bake(prep(rec), new_data = camels_train),
...
train_baked <- bake(rec_prep, new_data = camels_train)

# fit finalized workflow
best_nn <- select_best(nn_tuned, metric = "rsq")
...
The permutation importance results show that precipitation (p_mean) and aridity were by far the most influential predictors in the neural network model, with importance scores of approximately 0.56 and 0.46 respectively. This is hydrologically reasonable because streamflow is strongly controlled by water input and the balance between precipitation and evaporative demand. Slope also contributed moderately to predictions, suggesting that watershed topography influences runoff generation and drainage efficiency, while elevation, sand fraction, and the interaction term had comparatively smaller effects. Several of the most important predictors, particularly precipitation and aridity, are also likely to exhibit relatively high spatial autocorrelation because climate patterns vary systematically across geographic regions of the United States. This means some of their predictive power may partially reflect geographic structure in the dataset, acting as geographic proxies in addition to representing real hydrologic mechanisms. However, because the spatial cross-validation performance only declined moderately relative to random cross-validation, the results suggest that these predictors contain meaningful transferable environmental signal rather than relying entirely on location.
...
The neural network model does not provide built-in variable importance measures in the same way as models such as random forests, which represents an interpretability trade-off accepted in exchange for improved predictive flexibility and performance. To address this limitation, permutation importance using vip::vi_permute() was applied to evaluate how model performance changed when each predictor was shuffled. Additional interpretability approaches that could further diagnose predictor influence include partial dependence plots or accumulated local effects plots to better understand nonlinear relationships and interactions learned by the neural network.
...
ggplot(
  ...
  scale_color_viridis_c() +
  labs(
    title = "Final Neural Network Model: Observed vs Predicted Streamflow",
    x = "Observed logQmean",
...
The finalized neural network model performed well on the held-out test set, with performance remaining similar to the spatial cross-validation estimates. Predictions generally followed the 1:1 line, indicating strong agreement between observed and predicted streamflow values. The model tended to under predict some lower flow basins, while predictions for higher flow basins were more closely aligned with the 1:1 line and occasionally slightly over predicted. This pattern suggests the model captured major hydrologic gradients well but had more difficulty distinguishing subtle variation among low-flow watersheds. Future improvements could include incorporating additional watershed characteristics or regional predictors to better represent variability in low streamflow basins.


# Rubric

 - [ x] **Q1**:  Download data (15 pts)
 - [ x] **Q2**:  Make 2 maps (20 pts)
 - [ x] **Q3**:  Add xgboost + neural network to the workflow set (40 pts)
 - [x ] **Q4a**: Data prep & EDA (30 pts)
 - [x ] **Q4b**: Recipe with justification (30 pts)
 - [ x] **Q4c**: Define 3 models (30 pts)
 - [ x] **Q4d**: Workflow set + spatial CV comparison (35 pts)
 - [ x] **Q4e**: Tuning (named grid, ≥ 2 hyperparameters, autoplot) (25 pts)
 - [x ] **Q4f**: VIP interpretation with geographic-proxy lens (25 pts)
 - [ x] **Q4g**: `last_fit()` evaluation & plot (25 pts)
 - [x ] **Well-structured, compelling Qmd deployed as web page** (25 pts)
