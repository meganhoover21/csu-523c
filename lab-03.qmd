...
knitr::include_graphics('04-dams.jpg')
```
...
counties<- AOI::aoi_get(state = "conus", county = "all") |>
 ...
 st_crs(counties)# now in 5070 Albers Equal Area
```
...
```{r}
count_cen<-st_centroid(counties)

# combine ALL points into ONE MULTIPOINT
count_union <- st_union(count_cen)

plot(count_union) #should be a cloud of dots
plot(count_cen) #should be dots
...
sq_grid =st_make_grid(counties, n=c(70)) |>
  st_sf() |>
  mutate(id=1:n()) #created identifier from 1: however manycells
...
tm_shape(counties) + tm_polygons (col= "grey") + tm_shape (hex_grid) + tm_polygons (col= "red")

#voronoi

v_grid= st_voronoi(count_union) |>
  st_cast() |>
  st_sf() |>
  mutate(id=1:n())

tm_shape(counties) + tm_polygons (col= "grey") + tm_shape (v_grid) + tm_polygons (col= "red")
  
#triangulate
t_grid = st_triangulate(count_union)|>
  st_cast() |>
  st_sf() |>
  mutate(id= 1:n())
#quick visual. looks good!
tm_shape(counties) + tm_polygons (col= "grey") + tm_shape (count_union) + tm_polygons (col= "black") + tm_shape(t_grid) + tm_polygons (col= "red")
```
...
```{r, eval = TRUE}
c_boundary <- st_union(counties) #now one polygon for the boundary.
...
```{r, eval = TRUE}
boundary_simplified<- rmapshaper::ms_simplify(c_boundary, keep = 0.05) #want to get the simplest shape that uses less data. Kept 5%

plot(boundary_simplified)
```
...
```{r}
#| eval: false
# Check: Is the simplified boundary valid? Yes
...
I was able to remove 10715 points. Removing excess points helps to remove data that's not needed and that can slow down your computer. However, if too many points are removed, it may not be a good representation. 

```{r}
mapview::npts(c_boundary) |>
  print()#we had 11,292
...
```{r}
v_grid <- st_intersection(v_grid, boundary_simplified)

t_grid <- st_intersection(t_grid, boundary_simplified)

sq_grid <- st_filter(sq_grid, boundary_simplified)

hex_grid<- st_filter(hex_grid, boundary_simplified)
```
...
```{r}
plot_sf <- function(arg1, arg2) {
  
  p <- ggplot() +
    geom_sf(data = arg1, fill = "white", color = "navy", size = 0.2) +
    ggtitle(arg2) +
    theme_void() +
    labs(caption = paste("Number of features:", nrow(arg1)))
...
```{r}
plot_sf(counties, "Counties") #number of fefatures in number of rows

plot_sf(v_grid, "Voronoi Tessellation")

plot_sf(t_grid, "Delaunay Triangulation")

plot_sf(sq_grid, "Square Grid")

plot_sf(hex_grid, "Hex Grid")
```
...
summary_table <- bind_rows(
  ...
  sum_tess(t_grid, "Delaunay"),
  sum_tess(sq_grid, "Square Grid"),
  sum_tess(hex_grid, "Hex Grid")
)
...
I combined this step and 2.2 in the code chunk above.
```{r, eval = FALSE}
#bind_rows(
...
summary_table |>
flextable() |>
  set_caption("Tesselation Summary Statistics") |>
  autofit()

summary_table |>
  kable(digits = 2, caption = "Tessellation Summary Statistics") |>
...
The `counties` object has a moderate amount of rows/ n features. The mean area of polygon sizes are 2605 km^2, but the SD is much higher meaning that there are many counties that are either bigger or smaller than the mean. This could be due to county boundaries being determined by governments; therefore, they are not regularly shaped and spatial analysis may prove to be inconsistent. Point-in-polygon counts may be misleading, since larger counties will naturally contain more dams simply due to their size.*

The `Voronoi` tessellation has the same number of features as `counties` and a SD that is slightly higher than the mean area, but it has a lower variation in size than `counties`. These polygons are based on proximity to points, so they better represent spatial relationships. However, since their sizes still vary, some bias remains in how dam counts are distributed.*

The `Delaunay` triangulation creates many smaller polygons (6198 features) with a lower average mean area of 1290.37 km^2. This allows for more detailed detection of clustering, since each unit is smaller. However, it also increases computational cost and can introduce noise, since small polygons may contain very few dams. The SD is higher than the mean area, which could be a reason of introduced noise.

The `square and hex grids` both have zero standard deviation, meaning all tiles are the same size. This makes comparisons across space more consistent and reduces size-based bias from MAUP. However, because these grids are placed arbitrarily, they can split real clusters across boundaries.

Overall, tessellations with uniform size (square and hex grids) provide more consistent comparisons, while those with variable sizes (counties, Voronoi, Delaunay) can introduce bias due to differences in polygon area.
...
#remove na values in lat and long columns, transform to sf, and transform crs, filter to only us boundary
dams2<- dams |> filter(!is.na(latitude),
                      !is.na(longitude)) |>
...
```{r}
pip_count <- function(points, polygons, id_col) {
  
  if (st_crs(points) != st_crs(polygons)) {
    points <- st_transform(points, st_crs(polygons))
  }
  
  counts <- st_join(points, polygons) |>
    st_drop_geometry() |>
    count(.data[[id_col]], name = "n")
  
  polygons |>
    left_join(counts, by = setNames(id_col, id_col)) |>
...
```{r}
counties_pip <- pip_count(dams2, counties, "fip_code")
v_grid_pip   <- pip_count(dams2, v_grid, "id")
t_grid_pip   <- pip_count(dams2, t_grid, "id")
sq_grid_pip  <- pip_count(dams2, sq_grid, "id")
hex_grid_pip <- pip_count(dams2, hex_grid, "id")
```
...
```{r}
plot_pip <- function(arg1, arg2) {
  
  ggplot() +
    geom_sf(data = arg1, aes(fill = n), color = NA) + #colors polygons by number of dams, removes borders (color= NA)
    scale_fill_viridis_c() +
    ggtitle(arg2) +
    theme_void() +
    labs(caption = paste("Total dams:", sum(arg1$n))) #total dams in that tessellation
}
```
...
#verify dams across all have same amt. They don't
sum(counties_pip$n)
sum(v_grid_pip$n)
sum(t_grid_pip$n)
sum(sq_grid_pip$n)
sum(hex_grid_pip$n)
plot_pip(counties_pip, "Number of Dams Across Counties")

plot_pip(v_grid_pip, "Voronoi Surface: Number of Dams Across Counties")

plot_pip(t_grid_pip, "Delaunay Surface: Number of Dams Across Counties")

plot_pip(sq_grid_pip, "Square Grid Surface: Number of Dams Across Counties")

plot_pip(hex_grid_pip, "Hex Grid Surface: Number of Dams Across Counties")
```
...
The five tessellations produce noticeably different patterns of dam clustering, even though they are based on the same dataset. `Counties` show very uneven patterns, especially in the western United States where large counties appear to contain many dams simply due to their size. In contrast, the `square and hex grids` produce more uniform visual patterns because their tile sizes are consistent (standard deviation near zero in Q2). The `Voronoi and Delaunay` tessellations reveal more localized clustering, particularly because they are based on spatial proximity and, in the case of Delaunay, much smaller polygon sizes (over 6000 features with a mean area of ~1290 km²).

In contrast, the `square and hexagonal grids` divide the same space into equal-sized units, which spreads dam counts across multiple smaller tiles. As a result, areas like Nevada and Arizona appear less concentrated in these tessellations compared to `counties`.However, the `Delaunay tessellation` reveals smaller, localized clusters that are not visible in the county map. This difference is driven by scale: larger units smooth patterns, while smaller units reveal detail. In the southeastern U.S., `square grids` sometimes break up clusters into separate tiles due to their rigid alignment, making patterns appear more fragmented. The `hexagonal grid`, with its equal area and more natural neighbor structure, produces smoother and more continuous clusters in this region.

These differences are a clear example of the modifiable areal unit problem (MAUP), where changing the size and shape of spatial units changes the results of the analysis. From Q2, `counties and Voronoi polygons` have high variability in area (high standard deviation), which can bias results because larger polygons tend to contain more dams. In contrast, `square and hex grids` eliminate this size-based bias but introduce arbitrary boundaries that may split real-world clusters. The slight differences in total dam counts across tessellations also reflect how spatial predicates and boundary simplification affect whether points are included or excluded, especially near edges.

For decision-making, the `hexagonal grid` is the best choice. It provides equal-area units, reducing bias from polygon size, and its geometry produces smoother and more interpretable clustering patterns compared to `square grids`. It also avoids the over-fragmentation seen in `Delaunay triangulation`, which, while detailed, can be computationally expensive and harder to interpret. While `counties` align with administrative boundaries, they are less useful for identifying true spatial clustering. Overall, the`hex grid` is the most effective for supporting dam management decisions.

```{r}


(plot_pip(counties_pip, "Counties") |
  plot_pip(v_grid_pip, "Voronoi") |
  plot_pip(t_grid_pip, "Delaunay")) /
( plot_pip(sq_grid_pip, "Square Grid") |
  plot_pip(hex_grid_pip, "Hex Grid"))
```
...
unique(dams2$purposes) %>% length() #574
```
...
The result of this would indicate: There's a lot of NA's. Aside from that, recreation seems to be the main purpose.
...
```{r}
hydro_hex <- dams2 |> filter(grepl("H", purposes))
hydro_hex<- pip_count(hydro_hex, hex_grid, "id")

irrig_hex <- pip_count(
  dams2 |> filter(grepl("I", purposes)),
  hex_grid,
  "id")

flood_hex <- pip_count(
  dams2 |> filter(grepl("C", purposes)),
  hex_grid,
  "id")

water_hex <- pip_count(
  dams2 |> filter(grepl("S", purposes)),
  hex_grid,
  "id")
```
...
```{r}
#adding gghighlight and filter
plot_pip_highlight <- function(arg1, arg2) {
  
  plot_pip(arg1, arg2) +
    gghighlight(n > mean(arg1$n) + sd(arg1$n))}

plot_pip_highlight(hydro_hex, "Areas with the most Hydroelectric dams") #most in CALI and New York area
plot_pip_highlight(irrig_hex, "Areas with the most Irrigation dams") #Colorado has the most. CALI, GA,MO/KS has some. Seems to follow river basin pattern?
plot_pip_highlight(flood_hex, "Areas with the most Flood Control dams") #KS seems to have the most? TX second most.
plot_pip_highlight(water_hex, "Areas with the most Water Supply dams") #WY,NE, IA, KS area has the most
```
...
These patterns generally make physical sense because dam placement is strongly controlled by hydrology, topography, and land use. Hydroelectric dams are strongly concentrated in California and parts of New York, which makes sense given the combination of mountainous terrain, and high precipitation in some regions. Irrigation dams are most prominent in Colorado and across the western interior (including California, Georgia, Missouri, and Kansas), which aligns with agricultural water demand and reliance on managed water systems in semi-arid regions. Flood control dams are concentrated in Kansas and Texas, possibly reflecting the need to manage periodic high-intensity rainfall and flood risk in flat, highly variable climate zones. Water supply dams cluster across Wyoming, Nebraska, Iowa, and Kansas, which reflects groundwater dependence and surface storage systems in regions without large natural lakes or consistent precipitation.

Hexagonal grids smooth spatial variation, which can dilute or redistribute clusters across tile edges. This can make some regions (like California or the Great Plains) appear less extreme because their density is averaged across more uniform spatial units. 
...
n_dams= ifelse(is.na(n_dams), 0, n_dams),
  ...
  mean_size = mean(surface_area_acres, na.rm = TRUE)
) |>
mutate(
  mean_year = ifelse(is.nan(mean_year), NA, mean_year),
  mean_size = ifelse(is.nan(mean_size), NA, mean_size)
) |>
  left_join(hex_grid, by = "id") |> #preserving empty polygons
#avoiding dropped tiles (critical for Moran’s I later)
   st_as_sf()
...
I notice that the majority of dams across the U.S. are from around ~1980. But in UT,AZ, and NY area they are mostly from 1900-1950. A lot of the most recent ones are in the IA, NE, KS area. I am not sure why my code for the size of the dams isn't working, but on my visual it shows that everywhere has a 0 mean size.

```{r}


map_grid_var <- function(data, var, title) {
  
  ggplot(data) +
    geom_sf(aes(fill = .data[[var]]), color = NA) + #dynamically pick columns
    scale_fill_viridis_c() +
    theme_void() +
    labs(title = title)  }

map_grid_var(grid_attr, "n_dams", "Dam Count by Tile")

map_grid_var(grid_attr, "mean_year", "Mean Dam Age by Tile") #majority of dams around ~1980. But in UT,AZ, and NY area they are mostly from 1900-1950. A lot of the most recent ones are in the IA, NE, KS area.

map_grid_var(grid_attr, "mean_size", "Mean Dam Surface Area by Tile")
```
...
W <- st_touches(hex_grid, sparse = FALSE)
diag(W) <- 0
W <- W / rowSums(W)
W[is.na(W)] <- 0

#create vectors
x_count <- grid_attr$n_dams
x_age   <- grid_attr$mean_year
x_size  <- grid_attr$mean_size

#remove na
x_age[is.na(x_age)] <- mean(x_age, na.rm = TRUE)
x_size[is.na(x_size)] <- mean(x_size, na.rm = TRUE)
...
data.frame(
  attribute = c("count", "age", "size"),
  correlation = c(
    cor(x_count, lag_count),
    cor(x_age, lag_age),
    cor(x_size, lag_size)
  )) 
...
The number of dams is positively correlated with how many other dams are spatially correlated in relation to the number of dams. The age of the dam is somewhat correlated to the age of the surrounding dams. The mean size of dams doesn't appear to be correlated with the size of neighboring dams.
...
```
The Moran scatterplot shows a clear positive relationship between dam density and spatial lag values, indicating spatial clustering of infrastructure. Most observations fall in the HH and LL quadrants, with relatively few spatial outliers, suggesting that dam distribution is spatially structured rather than random.
