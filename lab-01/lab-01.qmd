...
#| fig-align: center
#| out-width: "100%"
knitr::include_graphics('lab-01/img/colorado-river.jpg')
```
...
1. **What is the key?** Which column(s) connect the two tables? The site_no column
2. **What is the relationship?** One-to-one, or one-to-many? One to many; there's one key in the site_meta data but many duplicate site_no's in the discharge data (clean df).
...
clean |> count(site_no) |> filter(n > 1)
```
There aren't any rows that are duplicates in site_meta site_no column, but there are many duplicates of site_no's in the discharge (df= clean) data.
...
```{r}
q_meta<-left_join(clean,site_meta, by= "site_no")
```
...
```{r}
nrow(q_meta) == nrow(clean) #TRUE
sum(is.na(q_meta$drain_area_va)) == 0 #TRUE
q_meta |>                             # known value check. We know Lee                                       ferry site has 111,800 sq mi.
  filter(site_no == "09380000") |>
  distinct(site_no, drain_area_va) #is expected sq mi.
```
...
q_meta<- q_meta |> mutate( 
  flow_per_sqmi= Flow/drain_area_va) #cfs/sq mi
```
...
```{r}
 flag_q<-flag_q |>
  mutate(below_threshold = flow_7day < q10)
```
...
> Below-threshold days decreased between 2011-2023 by about 10 days. However, the amount of dry years has been consistent throughout 2000-2023. This implies that meeting water delivery obligations is tricky, because there consistently been a lack of discharge from precipitation.
...
nrow(annual_anomaly) == nrow(annual1)   # Should be TRUE
sum(is.na(annual_anomaly$base_mean_annflow)) # Should be 0
sum(is.na(annual_anomaly$base_sd_annflow))   # Should be 0
